...
import 'package:concard/Models/Indiviuals/search_conversation_model.dart';
import 'package:concard/Models/Indiviuals/search_conversation_model.dart';
...
class _MessagesScreenState extends State<MessagesScreen> {
  ...
  // SearchConversationModel? searchConversationModel;
  ConversationsListModel? searchConversationsList;
  String? searchValue;
          ...
          child: Stack(
            children: [
              ...
              Container(
                  ...
                  child:
                      // searchConversationsList == null
                      // ?
                      FutureBuilder<ConversationsListModel?>(
                          future: searchValue == null
                              ? ChatController().getConversationsList()
                              : ChatController().searchConversationsList(
                                  searchValue.toString().toLowerCase().trim()),
                          builder: (context, snapshot) {
                            if (snapshot.hasData) {
                              ConversationsListModel? conversationsList =
                                  snapshot.data;
                              return SizedBox(
                                  height: size.height * 1.0,
                                  width: size.width,
                                  child: conversationViewBox(
                                      size, conversationsList));
                            }
                            return const ShimmerLoadWidget();
                          })
                  // : conversationViewBox(size, searchConversationsList)
                  )
            ],
          ),
        ));
  ...
  Widget conversationViewBox(
      Size size, ConversationsListModel? conversationsList) {
    return conversationsList!.count != 0
        ? ListView.builder(
            ...
            itemBuilder: (context, index) {
              String? passTime = DateTimeManueplate().giveDifferenceInTime(
                  DateTime.parse(conversationsList
                        ...
                        Row(
                          children: [
                            CircleAvatar(
                                ...
                                backgroundImage: NetworkImage(conversationsList
                                    .conversations![index].recipient!.image!)),
        ...
        : const Center(child: Text("There is no converstions"));
  }
}
